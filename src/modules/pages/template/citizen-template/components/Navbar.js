      ...
      {isMenuOpen ? (
        <div className="lg:hidden font-semibold">
          <NavLink
            ...
            className={({ isActive }) => (isActive ? activeLink : normalLink)}
          >
            <div className="block text-center my-2">{isKhmer ? "ទំព័រដើម" : "Home"}</div>
          </NavLink>
          {categoryList.map((category) => (
            <NavLink
              key={category.id}
              ...
              to={`/${id}/template/citizen/citizen-item/${category.id}`}
            >
              <div className="block text-center my-2">{isKhmer ? category.khName : category.name}</div>
            </NavLink>
          ))}
          ...
          <div className="flex justify-center">
            <Dropdown
              label={<Avatar img={changeLanguageIcon} alt="Change Language" className="cursor-pointer" />}
              inline
            >
              {languages.map((lng) => (
                <Dropdown.Item
