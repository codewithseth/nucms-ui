    ...
    <Table.Body>
      <div className="flex flex-col gap-10">
        ...
        {images.map((image, index) => {
          const imagePath = `/${user?.id}/template${image.split(".").slice(0, -1)}`;
          const previewPath = `/preview/template${image.split(".").slice(0, -1)}`;
          ...
          return (
            <div key={index} className="px-2 pl-0 pr-2 relative group">
              <div className="hover w-full h-auto relative overflow-hidden">
                <a href={imagePath} target="_blank" rel="noopener noreferrer" className="block h-full">
